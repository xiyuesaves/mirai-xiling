...
class xiling {
    constructor(options) {
        ...
        if (options.autoLoadPlugin === undefined) {
            this.options.autoLoadPlugin = true;
        }
    ...
    // 优先功能判断
    prerequisite(msg) {
        for (var key in this.preFuns) {
            if (this.preFuns[key](msg)) {
                return false;
            }
        }
        return true;
    ...
    autoLoadPlugin() {
        fs.stat("./plugin", (err, stats) => {
            if (!err && stats.isDirectory()) {
                console.log("[汐灵] 开始自动加载插件");
                fs.readdir("./plugin", (err, files) => {
                    files.forEach(item => {
                        console.log(join(process.cwd(),"./plugin/",item))
                        this.use(require(join(process.cwd(),"./plugin/",item)));
                    })
                })
            } else {
                console.log("[汐灵] 没有找到插件文件夹")
            }
        });
    }
    // 加载插件
    use(plugin) {
        ...
        //      最高优先级  主动功能 被动功能 加载时触发 插件名
        const { priority, command, passive, mounted, name } = plugin;
    ...
    onMsg(msg) {
        ...
        if (msg.isAt()) {
            let errMsg = "";
            if (msg.sender.id === this.options.dev) {
                errMsg = this.db.prepare("SELECT reply_text FROM default_err_static WHERE err_name = ?").all("master_comment");
            } else {
                errMsg = this.db.prepare("SELECT reply_text FROM default_err_static WHERE err_name = ?").all("none_comment");
            }
            errMsg = errMsg[Math.floor(Math.random() * errMsg.length)].reply_text;
